...
[<Fact>]
let ``First CvParam`` () = CvParam.structuralEquality (invesWsParsed.Head :?> CvParam) allExpectedTerms[0]

[<Fact>]
let ``Empty investigation is parsed with all structural ontology terms in order`` () =
    Assert.All((List.zip invesWsParsed allExpectedTerms), (fun (expected,actual) ->
        CvParam.structuralEquality (expected :?> CvParam) actual
    ))
